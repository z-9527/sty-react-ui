...
class IndexList extends Component {
  ...
  componentDidMount() {
    ...
    this.sections = document.getElementsByClassName(
      `${this.props.prefixCls}-section`
    );
    this.firstTitle = document.getElementsByClassName(
      `${this.props.prefixCls}-section-title`
    )[0];
  }
    ...
    return React.Children.map(children, (item, num) => {
      ...
      if (item.type === Section) {
        return (
          <li
            data-num={num}
            onClick={() => this.scrollToElement(num)}
            className={classnames(sidebarcls)}
          >
            {' '}
            {index}
            {activeIndex === index && moving && (
              <div className={`${prefixCls}-sidebar-item-moving`}>
                {index.slice(0, 1)}
              </div>
            )}
          </li>
  ...
  scrollToElement = num => {
    const maxScroll =
      document.documentElement.offsetHeight - window.innerHeight; // 页面最大滚动距离
